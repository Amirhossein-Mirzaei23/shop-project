...
<br>
    ...
    <div  class="w-10/12 mx-auto h-full grid grid-cols-3 mt-8 " :style="`background-image:url()`">
      ...
      <div id="carsoulImg" class="p-2 bg-slate-400 m-auto rounded-lg">
       <img  @mousemove="xClient" @mouseleave="DefaultImageMargin" :style="{ marginLeft:`${x}px`,marginTop:`${y}px`}" :src="singleProduct.images[0]"  class="max-w-80 w-80 max-h-80 m-auto" />
      </div>
    </div>
      
      <div class="col-span-2 h-5/6 w-11/12 m-auto grid grid-rows-4">

      <h1 class="text-right font-serif text-5xl">{{singleProduct.title}}</h1>
     
      <p class="text-slate-600 font-thin text-left row-span-2 mt-5 ">{{singleProduct.description}}</p>
      
     <div class="grid grid-cols-3">
      <p class=" w-1/4 h-min font-sans text-center text-5xl text-red-600 shadow-sm shadow-teal-600">{{singleProduct.price}}<span class="text-3xl">$</span></p>
      <div>
   ...
   <button class="bg-red-900 rounded-xl p-1" @click="nextt">next</button>
       <button class="bg-red-900 rounded-xl p-1" @click="prevv">Previos</button>
  </div>
</template>
