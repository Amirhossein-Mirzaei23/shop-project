...
<div class="container w-full h-72 mt-3 bg-opacity-30 bg-gray-400 flex items-center justify-end text-xs sm:text-xs lg:text-lg">
  ...
  <div class="w-11/12 h-full pr-3 flex justify-end items-center">
    ...
    <div v-show="keyword" class=" h-max-40 overflow-hidden z-10">
      ...
      <ul :key="keyword" class="w-11/12 h-24 h-max-40 mx-auto rounded-xl bg-slate-100 overflow-auto">
        
      <div v-for="index in search.data" :key="index">
        
        <search-bar-founded-item :index="index"  />
      </div>
      </ul>
...
async function keyWordSearch(event){
  ...
  keyword.value=event.target.value
   search= await useFetch(`https://api.escuelajs.co/api/v1/products/?title=${keyword.value}`)
   console.log(search);
  if(keyword){
//ul.class add hidden
  }
...
///  watch(keyword,async ()=>{
/// 
/// let url=(`https://api.escuelajs.co/api/v1/products/?title=${keyword.value}`)
