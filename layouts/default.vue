...
<!---navbar logo and Icon container--->
    ...
    <img class="rounded-full bg-white w-7/12 h-full scale-125 mx-auto" src="../assets/images/gradient-shop-local-logo.avif" alt="logo">
    </div>
<!---navbarIcon--->
...
<!---navbar items--->          <!------>
  <div id="navbarContioner" class="sti w-full h-full grid grid-cols-4 gap-1 items-center justify-items-center">
    <div class="w-8/12 h-5/6 flex items-center justify-items-center hover:shadow-xl hover:shadow-sky-400 border-b-2 hover:-translate-y-10 border-zinc-100 rounded-lg px-1 text-nowrap animate__animated animate__infinite animate__pulse animate__delay-2s animate__slow"><NuxtLink class="w-full h-full m-auto" to='/aboutUs'><button id="1" @mouseover="navbar.itemOne=true;"  @mouseleave="navbar.itemOne=false"   class="w-11/12 h-full mx-auto"><img v-if="navbar.itemOne" class="w-6/12 mx-auto hover:scale-110 hover:animate-spin" src="~/assets/images/navbar/call-centerpng.png" alt=""><p v-if="!navbar.itemOne">تماس با ما</p></button></NuxtLink></div>
    <div class="w-8/12 h-5/6 flex items-center justify-items-center hover:shadow-xl hover:shadow-sky-400 border-b-2 hover:-translate-y-10 border-zinc-100 rounded-lg px-1 text-nowrap animate__animated animate__infinite animate__pulse animate__delay-3s animate__slow"><NuxtLink class="w-full h-full m-auto" to='/recruitmentpage'><button id="2" @mouseover="navbar.itemtwo=true  " @mouseleave="navbar.itemtwo=false"   class="w-11/12 h-full mx-auto text-center"><img v-if="navbar.itemtwo" class="w-6/12 mx-auto hover:scale-110 hover:animate-spin"  src="~/assets/images/navbar/1693501.webp" alt=""><p v-if="!navbar.itemtwo">استخدام</p></button></NuxtLink></div>
    <div class="w-8/12 h-5/6 flex items-center  justify-items-center hover:shadow-xl hover:shadow-sky-400 border-b-2 hover:-translate-y-10 border-zinc-100 rounded-lg px-1 text-nowrap animate__animated animate__infinite animate__pulse animate__delay-4s animate__slow">
    <NuxtLink class="w-full h-full" to="/"><button id="3" @mouseover="navbar.itemthree=true" @mouseleave="navbar.itemthree=false" class="w-11/12 h-full mx-auto"><img v-if="navbar.itemthree" class="w-6/12 mx-auto hover:scale-110 hover:animate-spin" src="~/assets/images/navbar/shopStoreIcon.png" alt=""><p v-if="!navbar.itemthree">فروشگاه</p></button></NuxtLink></div>
